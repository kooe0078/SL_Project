...
ATutorialManager::ATutorialManager()
{
    ...
    targetStaticMesh = CreateDefaultSubobject<UStaticMeshComponent>(TEXT("targetStaticMesh"));
    targetStaticMesh->SetupAttachment(sceneRootComp);

    targetBoxCollision = CreateDefaultSubobject<UBoxComponent>(TEXT("targetBoxCollision"));
    targetBoxCollision->SetupAttachment(targetStaticMesh);
...
void ATutorialManager::BeginPlay()
{
    ...
    targetBoxCollision->OnComponentBeginOverlap.AddDynamic(this, &ATutorialManager::TargetBoxCollisionOnOverlapBegin);
}
...
void ATutorialManager::TargetBoxCollisionOnOverlapBegin(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
    AMainCharArrow* mainCharArrow = Cast<AMainCharArrow>(OtherActor);

    if (mainCharArrow)
    {
        if (tutorialMissionCheck == ETutorialMissionCheck::MissionCheck3)
        {
            MissionSuccess(0.5f);
        }
    }
